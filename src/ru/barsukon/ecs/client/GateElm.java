...
abstract class GateElm extends CircuitElm {
    ...
    public GateElm(int xx, int yy) {
        ...
        if (lastSchmitt) flags |= FLAG_SCHMITT;
    ...
    void setPoints() {
        ...
        if (dn > 150 && this == sim.dragElm) setSize(2);
        int hs = gheight;
        int i;
        ww = gwidth2; // was 24
        if (ww > dn / 2) ww = (int) (dn / 2);
        if (isInverting() && ww + 8 > dn / 2) ww = (int) (dn / 2 - 8);
        calcLeads(ww * 2);
        inPosts = new Point[inputCount];
        inGates = new Point[inputCount];
        allocNodes();
        int i0 = -inputCount / 2;
        if (hasFlag(FLAG_INVERT_INPUTS)) icircles = new Point[inputCount];
        else icircles = null;
        for (i = 0; i != inputCount; i++, i0++) {
            if (i0 == 0 && (inputCount & 1) == 0) i0++;
            ...
            if (icircles != null) icircles[i] = interpPoint(lead1, lead2, -4 / (ww * 2.), hs * i0);
            volts[i] = (lastOutput ^ isInverting()) ? 5 : 0;
        }
        hs2 = gwidth * (inputCount / 2 + 1);
        setBbox(point1, point2, hs2);
        if (hasSchmittInputs()) schmittPoly = getSchmittPolygon(gsize, .47f);
    }
    ...
    void draw(Graphics g) {
        ...
        } else drawGatePolygon(g);
        g.setLineWidth(2);
        if (hasSchmittInputs()) drawPolygon(g, schmittPoly);
        g.setLineWidth(1);
        if (linePoints != null) for (i = 0; i != linePoints.length - 1; i++) drawThickLine(g, linePoints[i], linePoints[i + 1]);
        if (isInverting()) drawThickCircle(g, pcircle.x, pcircle.y, 3);
        if (icircles != null) for (i = 0; i != inputCount; i++) drawThickCircle(g, icircles[i].x, icircles[i].y, 3);
    ...
    boolean getInput(int x) {
        ...
        if (!hasSchmittInputs()) return (volts[x] > highVoltage * .5) ? high : !high;
        ...
        if (lastTime != sim.t) {
            ...
            if (lastOutput == !f) {
                if (oscillationCount++ > 50) {
                    ...
                    oscillationCount = 0;
                    if (sim.getrand(10) > 5) f = lastOutput;
                }
    ...
    public EditInfo getEditInfo(int n) {
        if (n == 0) return new EditInfo("# of Inputs", inputCount, 1, 8).setDimensionless();
        if (n == 1) return new EditInfo("High Logic Voltage", highVoltage, 1, 10);
        if (n == 2) return EditInfo.createCheckbox("Schmitt Inputs", hasSchmittInputs());
        if (n == 3) return EditInfo.createCheckbox("Invert Inputs", hasFlag(FLAG_INVERT_INPUTS));
        return null;
    }
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 2) {
            if (ei.checkbox.getState()) flags |= FLAG_SCHMITT;
            else flags &= ~FLAG_SCHMITT;
    ...
    double getCurrentIntoNode(int n) {
        if (n == inputCount) return current;
        return 0;
    }
}
