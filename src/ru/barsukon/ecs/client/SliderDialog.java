...
class SliderDialog extends Dialog {
            ...
            if (adj != null) {
                if (!adj.sliderBeingShared()) {
                    ...
                    for (j = 0; j != sim.adjustables.size(); j++) {
                        ...
                        if (adji.sharedSlider != null) break;
                        // don't share with ourselves
                        if (adji == adj) continue;
                        ch.add("Share Slider: " + adji.sliderText);
                        if (adji == adj.sharedSlider) ch.setSelectedIndex(ch.getItemCount() - 1);
                    }
                    vp.insert(ch, idx++);
                }
    ...
    void apply() {
        ...
        for (i = 0; i != einfocount; i++) {
            ...
            try {
                ...
                if (adj.label != null) adj.label.setText(adj.sliderText);
        ...
        for (i = 0; i != einfocount; i++) {
            ...
            if (ei.choice == src) {
                ...
                if (ei.choice.getSelectedIndex() == 0) {
                    ...
                    if (adj.sliderText == null || adj.sliderText.length() == 0) adj.sliderText = ei.name.replaceAll(" \\(.*\\)$", "");
                    adj.createSlider(sim, ei.value);
                } else {
                    int j;
                    int ct = 0;
    ...
    public void clearDialog() {
        while (vp.getWidget(0) != hp) vp.remove(0);
    }
}
