...
class EditDialog extends Dialog {
    ...
    void buildDialog() {
        ...
        for (i = 0;; i++) {
            ...
            if (einfos[i] == null) break;
            final EditInfo ei = einfos[i];
            String name = Locale.LS(ei.name);
            if (ei.name.startsWith("<")) vp.add(l = new HTML(name));
            else vp.add(l = new Label(name));
            if (i != 0 && l != null) l.setStyleName("topSpace");
            if (ei.choice != null) {
                vp.add(ei.choice);
                ei.choice.addChangeHandler(
                    new ChangeHandler() {
                ...
                vp.add(ei.checkbox);
                ei.checkbox.addValueChangeHandler(
    ...
    static String unitString(EditInfo ei, double v) {
        ...
        if (ei != null && ei.dimensionless) return noCommaFormat.format(v);
        if (Double.isInfinite(va)) return noCommaFormat.format(v);
        if (v == 0) return "0";
        if (va < 1e-12) return noCommaFormat.format(v * 1e15) + "f";
        if (va < 1e-9) return noCommaFormat.format(v * 1e12) + "p";
        if (va < 1e-6) return noCommaFormat.format(v * 1e9) + "n";
        if (va < 1e-3) return noCommaFormat.format(v * 1e6) + "u";
        if (va < 1/*&& !ei.forceLargeM*/) return noCommaFormat.format(v * 1e3) + "m";
        if (va < 1e3) return noCommaFormat.format(v);
        if (va < 1e6) return noCommaFormat.format(v * 1e-3) + "k";
        if (va < 1e9) return noCommaFormat.format(v * 1e-6) + "M";
    ...
    public void clearDialog() {
        while (mainPanel.getWidget(0) != bottomButtonPanel) mainPanel.remove(0);
    }

    public void closeDialog() {
        super.closeDialog();
        if (CirSim.editDialog == this) CirSim.editDialog = null;
        if (CirSim.customLogicEditDialog == this) CirSim.customLogicEditDialog = null;
    }
}
