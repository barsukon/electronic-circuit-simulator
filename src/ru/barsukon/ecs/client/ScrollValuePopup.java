...
public class ScrollValuePopup extends PopupPanel implements MouseOutHandler, MouseWheelHandler, MouseDownHandler {
    ...
    ScrollValuePopup(final int x, final int y, int dy, CircuitElm e, CirSim s) {
        ...
        for (int i = 0; i < labMax; i++) {
            ...
            if (i == 2) labels[i].addStyleDependentName("selected");
            else if (i == 1 || i == 3) labels[i].addStyleDependentName("1off");
            else labels[i].addStyleDependentName("2off");
            vp.add(labels[i]);
        }
        doDeltaY(dy);
    ...
    private void setupValues() {
        ...
        for (int i = 0; i < nvalues + 1; i++) {
            ...
            if (currentvalue < values[i]) { // overshot - need to insert value
                currentidx = i;
                for (int j = nvalues - 1; j >= i; j--) values[j + 1] = values[j];
    ...
    private void setupLabels() {
        ...
        for (int i = 0; i < labMax; i++) {
            ...
            if ((thissel + i - 2) < 0 || (thissel + i - 2) >= nvalues) labels[i].setText("---");
            else {
                labels[i].setText(CircuitElm.getShortUnitText(values[thissel + i - 2], ""));
                if (thissel + i - 2 == currentidx) labels[i].addStyleDependentName("current");
            }
        }
    }
    ...
    public void onMouseDown(MouseDownEvent e) {
        if (e.getNativeButton() == NativeEvent.BUTTON_LEFT || e.getNativeButton() == NativeEvent.BUTTON_MIDDLE) close(true);
        else close(false);
    }
    ...
    public void doDeltaY(int dy) {
        ...
        if (currentidx + deltaY / scale < 0) deltaY = -scale * currentidx;
        if (currentidx + deltaY / scale >= nvalues) deltaY = (nvalues - currentidx - 1) * scale;
        setElmValue();
        setupLabels();
    }
