...
class ScopePlot {
    ...
    ScopePlot(CircuitElm e, int u, int v, double manS) {
        ...
        if (units == Scope.UNITS_OHMS || units == Scope.UNITS_W) manVPosition = -Scope.V_POSITION_STEPS / 2;
    }
    ...
    void reset(int spc, int sp, boolean full) {
        ...
        scopePointCount = spc;
        if (scopePlotSpeed != sp) oldSpc = 0; // throw away old data
        scopePlotSpeed = sp;
        ...
        } else lastUpdateTime = CirSim.theSim.t;
        ptr = 0;
    }
...
class Scope {
    ...
    void showCurrent(boolean b) {
        showI = b;
        if (b && !showingVoltageAndMaybeCurrent()) setValue(0);
        calcVisiblePlots();
    }
    ...
    void setManualScale(boolean value, boolean roundup) {
        if (value != manualScale) clear2dView();
        manualScale = value;
    ...
    void resetGraph(boolean full) {
        scopePointCount = 1;
        while (scopePointCount <= rect.width) scopePointCount *= 2;
        if (plots == null) plots = new Vector<ScopePlot>();
        showNegative = false;
        int i;
        for (i = 0; i != plots.size(); i++) plots.get(i).reset(scopePointCount, speed, full);
        calcVisiblePlots();
    ...
    void setManualScaleValue(int plotId, double d) {
        if (plotId >= visiblePlots.size()) return; // Shouldn't happen, but just in case...
        clear2dView();
    ...
    void initialize() {
        ...
        if (!loadDefaults()) {
            ...
            for (i = 0; i != plots.size(); i++) {
                ...
                if (plot.units == UNITS_V) showV = true;
                if (plot.units == UNITS_A) showI = true;
            }
        }
    }
    ...
    void setRect(Rectangle r) {
        ...
        this.rect = r;
        if (this.rect.width != w) resetGraph();
    }
    ...
    void setValue(int val) {
        if (plots.size() > 2 || plots.size() == 0) return;
        CircuitElm ce = plots.firstElement().elm;
        if (plots.size() == 2 && plots.get(1).elm != ce) return;
        plot2d = plotXY = false;
        setValue(val, ce);
    }
    ...
    void addValue(int val, CircuitElm ce) {
        if (val == 0) {
            ...
            if (ce != null && sim.dotsCheckItem.getState() && !(ce instanceof OutputElm || ce instanceof ProbeElm)) plots.add(
                new ScopePlot(ce, UNITS_A, VAL_CURRENT, getManScaleFromMaxScale(UNITS_A, false))
            );
        } else {
            int u = ce.getScopeUnits(val);
            plots.add(new ScopePlot(ce, u, val, getManScaleFromMaxScale(u, false)));
            if (u == UNITS_V) showV = true;
            if (u == UNITS_A) showI = true;
        }
        calcVisiblePlots();
        resetGraph();
    }
    ...
    boolean showingVoltageAndMaybeCurrent() {
        ...
        for (i = 0; i != plots.size(); i++) {
            ...
            else if (sp.value != VAL_CURRENT) return false;
        }
        return gotv;
    }
	...
	*/
    ...
    int separate(Scope arr[], int pos) {
        ...
        for (i = 0; i != visiblePlots.size(); i++) {
            if (pos >= arr.length) return pos;
            Scope s = new Scope(sim);
            ScopePlot sp = visiblePlots.get(i);
            if (lastPlot != null && lastPlot.elm == sp.elm && lastPlot.value == VAL_VOLTAGE && sp.value == VAL_CURRENT) continue;
    ...
    void timeStep() {
        int i;
        for (i = 0; i != plots.size(); i++) plots.get(i).timeStep();
        ...
        if (plot2d && imageContext != null && plots.size() >= 2) {
            ...
            if (!isManualScale()) {
                ...
                x = (int) (rect.width * .499 +
                    (v / plots.get(0).manScale) * gridPx +
                    (gridPx * manDivisions * (double) (plots.get(0).manVPosition)) / (double) (V_POSITION_STEPS));
                y = (int) (rect.height * .499 -
                    (yval / plots.get(1).manScale) * gridPx -
    ...
    */
    ...
    void maxScale() {
        ...
        // the amplitude varies over time but you can't see that if the scale is
    ...
    void drawFFT(Graphics g) {
        if (fft == null || fft.getSize() != scopePointCount) fft = new FFT(scopePointCount);
    ...
    void drawSettingsWheel(Graphics g) {
        ...
        if (showSettingsWheel()) {
            g.context.save();
            if (cursorInSettingsWheel()) g.setColor(CircuitElm.selectColor);
    ...
    void draw2d(Graphics g) {
        ...
        if (!plotXY) g.setColor(Color.yellow);
        ...
        if (!sim.dialogIsShowing() && rect.contains(sim.mouseCursorX, sim.mouseCursorY) && plots.size() >= 2) {
            ...
            if (isManualScale()) {
                xValue = px.manScale *
                ((double) (sim.mouseCursorX - rect.x - rect.width / 2) / gridPx -
                    (manDivisions * px.manVPosition) / (double) (V_POSITION_STEPS));
                yValue = py.manScale *
                ((double) (-sim.mouseCursorY + rect.y + rect.height / 2) / gridPx -
    ...
    void draw(Graphics g) {
        ...
        for (i = 0; i != UNITS_COUNT; i++) {
            reduceRange[i] = false;
            if (maxScale && !manualScale) scale[i] = 1e-4;
        }
        ...
        for (i = 1; i < visiblePlots.size(); i++) {
            if (visiblePlots.get(i).units != visiblePlots.get(0).units) allPlotsSameUnits = false; // Don't draw horizontal grid lines unless all plots are in same units
        }

        if ((allPlotsSameUnits || showMax || showMin) && visiblePlots.size() > 0) calcMaxAndMin(visiblePlots.firstElement().units);

        // draw volt plots on top (last), then current plots underneath, then everything else
        for (i = 0; i != visiblePlots.size(); i++) {
            if (visiblePlots.get(i).units > UNITS_A && i != selectedPlot) drawPlot(g, visiblePlots.get(i), allPlotsSameUnits, false, sel);
        }
        for (i = 0; i != visiblePlots.size(); i++) {
            if (visiblePlots.get(i).units == UNITS_A && i != selectedPlot) drawPlot(g, visiblePlots.get(i), allPlotsSameUnits, false, sel);
        }
        for (i = 0; i != visiblePlots.size(); i++) {
            if (visiblePlots.get(i).units == UNITS_V && i != selectedPlot) drawPlot(g, visiblePlots.get(i), allPlotsSameUnits, false, sel);
        }
        // draw selection on top.  only works if selection chosen from scope
        if (selectedPlot >= 0 && selectedPlot < visiblePlots.size()) drawPlot(
            g,
            visiblePlots.get(selectedPlot),
            allPlotsSameUnits,
            true,
            sel
        );
        ...
        if (plots.get(0).ptr > 5 && !manualScale) {
            for (i = 0; i != UNITS_COUNT; i++) if (scale[i] > 1e-4 && reduceRange[i]) scale[i] /= 2;
        }

        if ((properties != null) && properties.isShowing()) properties.refreshDraw();
    }
    ...
    void calcMaxAndMin(int units) {
        ...
        for (si = 0; si != visiblePlots.size(); si++) {
            ...
            for (i = 0; i != rect.width; i++) {
                ...
                if (maxV[ip] > maxValue) maxValue = maxV[ip];
                if (minV[ip] < minValue) minValue = minV[ip];
            }
        }
    }
    ...
    void calcPlotScale(ScopePlot plot) {
        ...
        for (i = 0; i != rect.width; i++) {
            int ip = (i + ipa) & (scopePointCount - 1);
            if (maxV[ip] > max) max = maxV[ip];
            if (minV[ip] < -max) max = -minV[ip];
        }
        // scale fixed at maximum?
        if (maxScale) gridMax = Math.max(max, gridMax);
    ...
    void drawPlot(Graphics g, ScopePlot plot, boolean allPlotsSameUnits, boolean selected, boolean allSelected) {
        ...
        if (ox != -1) g.drawLine(ox, maxy - oy, x + i - 1, maxy - oy); // Horizontal
    }
    ...
    void selectScope(int mouseX, int mouseY) {
        ...
        if (plot2d || visiblePlots.size() == 0) cursorTime = -1;
        else cursorTime = sim.t - sim.maxTimeStep * speed * (rect.x + rect.width - mouseX);
    ...
    void checkForSelection(int mouseX, int mouseY) {
        ...
        selectedPlot = best;
        if (selectedPlot >= 0) cursorUnits = visiblePlots.get(selectedPlot).units;
    }

    void drawCursor(Graphics g) {
        if (sim.dialogIsShowing()) return;
        if (cursorScope == null) return;
        ...
        if (showFFT && cursorScope == this) {
            ...
            if (cursorX < 0) cursorX = sim.mouseCursorX;
            info[ct++] = CircuitElm.getUnitText((maxFrequency * (sim.mouseCursorX - rect.x)) / rect.width, "Hz");
        } else if (cursorX < rect.x) return;

        if (visiblePlots.size() > 0) info[ct++] = CircuitElm.getTimeText(cursorTime);
    ...
    void drawCursorInfo(Graphics g, String[] info, int ct, int x, Boolean drawY) {
        ...
        if (drawY) g.drawLine(rect.x, sim.mouseCursorY, rect.x + rect.width, sim.mouseCursorY);
        g.setColor(sim.printableCheckItem.getState() ? Color.white : Color.black);
        int bx = x;
        if (bx < szw / 2) bx = szw / 2;
    ...
    boolean canShowRMS() {
        if (visiblePlots.size() == 0) return false;
    ...
    void drawRMS(Graphics g) {
        ...
        for (; i != rect.width; i++) {
            ...
            // switching polarity?
            if (state == 1) {
                if (maxV[ip] < mid) sw = true;
            } else if (minV[ip] > mid) sw = true;
    ...
    void drawScale(ScopePlot plot, Graphics g) {
        if (!isManualScale()) {
            ...
            if (rect.y + rect.height <= textY + 5) return;
            double x = 0;
    ...
    void drawFrequency(Graphics g) {
        ...
        if (freq != 0) drawInfoText(g, CircuitElm.getUnitText(freq, "Hz"));
    }
    ...
    void drawElmInfo(Graphics g) {
        ...
        getElm().getInfo(info);
        int i;
        for (i = 0; info[i] != null; i++) drawInfoText(g, info[i]);
    }
    ...
    void drawInfoTexts(Graphics g) {
        ...
        if (visiblePlots.size() == 0) {
            if (showElmInfo) drawElmInfo(g);
            return;
        }
        ScopePlot plot = visiblePlots.firstElement();
        if (showScale) drawScale(plot, g);
        //    	if (showMax || showMin)
        //    	    calcMaxAndMin(plot.units);
        if (showMax) drawInfoText(g, "Max=" + plot.getUnitText(maxValue));
        if (showMin) {
            int ym = rect.height - 5;
            g.drawString("Min=" + plot.getUnitText(minValue), 0, ym);
        }
        if (showRMS) drawRMS(g);
        if (showAverage) drawAverage(g);
        if (showDutyCycle) drawDutyCycle(g);
        String t = getScopeLabelOrText(true);
        if (t != null && t != "") drawInfoText(g, t);
        if (showFreq) drawFrequency(g);
        if (showElmInfo) drawElmInfo(g);
    }

    String getScopeText() {
        // stacked scopes?  don't show text
        if (stackCount != 1) return null;

        // multiple elms?  don't show text (unless one is selected)
        if (selectedPlot < 0 && getSingleElm() == null) return null;

        // no visible plots?
        if (visiblePlots.size() == 0) return null;

        ScopePlot plot = visiblePlots.firstElement();
        if (selectedPlot >= 0 && visiblePlots.size() > selectedPlot) plot = visiblePlots.get(selectedPlot);
        if (plot.elm == null) return "";
        else return plot.elm.getScopeText(plot.value);
    }
    ...
    String getScopeLabelOrText(boolean forInfo) {
        ...
        if (t == null) {
            ...
            if (forInfo && showElmInfo) return null;
            t = getScopeText();
            if (t == null) return "";
            return Locale.LS(t);
        } else return t;
    ...
    CircuitElm getSingleElm() {
        ...
        for (i = 1; i < plots.size(); i++) {
            if (plots.get(i).elm != elm) return null;
        }
        return elm;
    }
    ...
    String dump() {
        ...
        for (i = 0; i < plots.size(); i++) {
            ...
            if ((flags & FLAG_PERPLOTFLAGS) != 0) x += " " + Integer.toHexString(p.getPlotFlags()); // NB always export in Hex (no prefix)
        ...
        if (text != null) x += " " + CustomLogicModel.escape(text);
        return x;
    }
    ...
    void undump(StringTokenizer st) {
        ...
        if (value == VAL_POWER_OLD) value = VAL_POWER;
        ...
        if (scale[UNITS_V] == 0) scale[UNITS_V] = .5;
        if (scale[UNITS_A] == 0) scale[UNITS_A] = 1;
        ...
        if ((flags & FLAG_PLOTS) != 0) {
            ...
            try {
                ...
                if (u > UNITS_A) scale[u] = Double.parseDouble(st.nextToken());
                setValue(value);
                // setValue(0) creates an extra plot for current, so remove that
                while (plots.size() > 1) plots.removeElementAt(1);

                int plotFlags = 0;
                for (i = 0; i != sz; i++) {
                    if (hasPlotFlags) plotFlags = Integer.parseInt(st.nextToken(), 16); // Import in hex (no prefix)
                    if (i != 0) {
                ...
                while (st.hasMoreTokens()) {
                    if (text == null) text = st.nextToken();
                    else text += " " + st.nextToken();
                }
            } catch (Exception ee) {}
        } else {
            // old-style dump
        ...
        if (text != null) text = CustomLogicModel.unescape(text);
        plot2d = plot2dFlag;
        setFlags(flags);
    }
    ...
    void handleMenu(String mi, boolean state) {
        if (mi == "maxscale") maxScale();
        if (mi == "showvoltage") showVoltage(state);
        if (mi == "showcurrent") showCurrent(state);
        if (mi == "showscale") showScale(state);
        if (mi == "showpeak") showMax(state);
        if (mi == "shownegpeak") showMin(state);
        if (mi == "showfreq") showFreq(state);
        if (mi == "showfft") showFFT(state);
        if (mi == "logspectrum") logSpectrum = state;
        if (mi == "showrms") showRMS = state;
        if (mi == "showaverage") showAverage = state;
        if (mi == "showduty") showDutyCycle = state;
        if (mi == "showelminfo") showElmInfo = state;
        if (mi == "showpower") setValue(VAL_POWER);
        if (mi == "showib") setValue(VAL_IB);
        if (mi == "showic") setValue(VAL_IC);
        if (mi == "showie") setValue(VAL_IE);
        if (mi == "showvbe") setValue(VAL_VBE);
        if (mi == "showvbc") setValue(VAL_VBC);
        if (mi == "showvce") setValue(VAL_VCE);
        if (mi == "showvcevsic") {
            plot2d = true;
            plotXY = false;
        ...
        if (mi == "manualscale") setManualScale(state, true);
        if (mi == "plotxy") {
            plotXY = plot2d = state;
            if (plot2d) plots = visiblePlots;
            if (plot2d && plots.size() == 1) selectY();
            resetGraph();
        }
        if (mi == "showresistance") setValue(VAL_R);
    }
        ...
        while (true) {
            for (e++; e < sim.elmList.size(); e++) {
                ...
                if ((ce instanceof OutputElm || ce instanceof ProbeElm) && ce != plots.get(0).elm) {
                    yElm = ce;
                    if (plots.size() == 1) plots.add(new ScopePlot(yElm, UNITS_V));
                    else {
    ...
    CircuitElm getElm() {
        if (selectedPlot >= 0 && visiblePlots.size() > selectedPlot) return visiblePlots.get(selectedPlot).elm;
        return visiblePlots.size() > 0 ? visiblePlots.get(0).elm : plots.get(0).elm;
    }

    boolean viewingWire() {
        int i;
        for (i = 0; i != plots.size(); i++) if (plots.get(i).elm instanceof WireElm) return true;
        return false;
    }
    ...
    CircuitElm getYElm() {
        if (plots.size() == 2) return plots.get(1).elm;
        return null;
    }
    ...
    public double getManScaleFromMaxScale(int units, boolean roundUp) {
        ...
        if (roundUp) return ScopePropertiesDialog.nextHighestScale((2 * s) / (double) (manDivisions));
        else return (2 * s) / (double) (manDivisions);
    }
    ...
    static String exportAsDecOrHex(int v, int thresh) {
        ...
        if (v >= thresh) return "x" + Integer.toHexString(v);
        else return Integer.toString(v);
    }

    static int importDecOrHex(String s) {
        if (s.charAt(0) == 'x') return Integer.parseInt(s.substring(1), 16);
