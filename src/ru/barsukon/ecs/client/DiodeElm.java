...
class DiodeElm extends CircuitElm {
    ...
    String dumpModel() {
        if (model.builtIn || model.dumped) return null;
        return model.dump();
    }
    ...
    void getInfo(String arr[]) {
        if (model.oldStyle) arr[0] = "diode";
        else arr[0] = Locale.LS("diode") + " (" + modelName + ")";
        arr[1] = "I = " + getCurrentText(getCurrent());
        arr[2] = "Vd = " + getVoltageText(getVoltageDiff());
        arr[3] = "P = " + getUnitText(getPower(), "W");
        if (model.oldStyle) arr[4] = "Vf = " + getVoltageText(model.fwdrop);
    }
    ...
    public EditInfo getEditInfo(int n) {
        if (n == 0) {
            ...
            for (i = 0; i != models.size(); i++) {
                ...
                if (dm == model) ei.choice.select(i);
            }
            return ei;
        }
        if (n == 1) {
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 1 || n == 2) {
            ...
            if (newModel.isSimple()) newModel.setForwardVoltage();
        ...
        if (n == 3) {
            ...
            if (model.isSimple()) model.setForwardVoltage();
    ...
    void stepFinished() {
        ...
        if (Math.abs(current) > 1e12) sim.stop("max current exceeded", this);
    }
}
