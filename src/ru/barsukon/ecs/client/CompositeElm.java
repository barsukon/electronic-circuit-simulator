...
public abstract class CompositeElm extends CircuitElm {
    ...
    public void loadComposite(StringTokenizer stIn, String model, int externalNodes[]) {
        ...
        while (modelLinet.hasMoreTokens()) {
            ...
            if (stIn != null) {
                ...
                if (useEscape()) dumpedCe = CustomLogicModel.unescape(dumpedCe);
            ...
            if (newce instanceof GroundElm) ((GroundElm) newce).setOldStyle();
    ...
    public boolean nonLinear() {
        for (int i = 0; i < compElmList.size(); i++) if (compElmList.get(i).nonLinear()) return true;
        return false;
    }
    ...
    public boolean getConnection(int n1, int n2) {
        ...
        for (i = 0; i < connectedNodes.size(); i++) {
            ...
            for (int j = 0; j < cnLinks.size(); j++) {
                ...
                for (int k = 0; k != lelm.getConnectionNodeCount(); k++) // are they connected?
                if (k != link.num && lelm.getConnection(link.num, k)) {
                    ...
                    for (m = 0; m != nodes.length; m++) if (nodes[m] == kn && !connectedNodes.contains(m)) connectedNodes.add(m);
                }
            }
        }
        return false;
    ...
    public boolean hasGroundConnection(int n1) {
        ...
        for (i = 0; i < connectedNodes.size(); i++) {
            ...
            for (int j = 0; j < cnLinks.size(); j++) {
                CircuitNodeLink link = cnLinks.get(j);
                CircuitElm lelm = link.elm;
                if (lelm.hasGroundConnection(link.num)) return true;
                // loop through all other nodes this element has
                for (int k = 0; k != lelm.getConnectionNodeCount(); k++) // are they connected?
                if (k != link.num && lelm.getConnection(link.num, k)) {
                    int kn = lelm.getConnectionNode(k);
    ...
    public void reset() {
        for (int i = 0; i < compElmList.size(); i++) compElmList.get(i).reset();
    }
    ...
    public double getPower() {
        double power;
        power = 0;
        for (int i = 0; i < compElmList.size(); i++) power += compElmList.get(i).getPower();
        return power;
    }
    ...
    public void startIteration() {
        for (int i = 0; i < compElmList.size(); i++) compElmList.get(i).startIteration();
    }

    public void doStep() {
        for (int i = 0; i < compElmList.size(); i++) compElmList.get(i).doStep();
    }

    public void stepFinished() {
        for (int i = 0; i < compElmList.size(); i++) compElmList.get(i).stepFinished();
    }
    ...
    public void delete() {
        for (int i = 0; i < compElmList.size(); i++) compElmList.get(i).delete();
        super.delete();
    }
    ...
    public void setCurrent(int vsn, double c) {
        for (int i = 0; i < voltageSources.size(); i++) if (voltageSources.get(i).vsNode == vsn) {
