...
public class Adjustable implements Command {
    ...
    boolean createSlider(CirSim sim) {
        ...
        if (sharedSlider != null) return true;
        if (sliderText.length() == 0) return false;
    ...
    public void execute() {
        ...
        for (i = 0; i != sim.adjustables.size(); i++) {
            ...
            if (adj == this || adj.sharedSlider == this) adj.executeSlider();
        }
    }
    ...
    boolean sliderBeingShared() {
        ...
        for (i = 0; i != CirSim.theSim.adjustables.size(); i++) {
            ...
            if (adj.sharedSlider == this) return true;
        }
        return false;
    }
    ...
    String dump() {
        int ano = -1;
        if (sharedSlider != null) ano = CirSim.theSim.adjustables.indexOf(sharedSlider);

        return (
            elm.sim.locateElm(elm) +
            " F1 " +
            editItem +
            " " +
            minValue +
