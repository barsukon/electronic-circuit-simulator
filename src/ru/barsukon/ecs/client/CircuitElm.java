...
public abstract class CircuitElm implements Editable {
    ...
    static CirSim sim;
    public static Color whiteColor, lightGrayColor, selectColor;
    public static Color positiveColor, negativeColor, neutralColor, currentColor;
    ...
    static void initClass(CirSim s) {
        ...
        if (stor != null) {
            ...
            if (s1 != null) decimalDigits = Integer.parseInt(s1);
            if (s2 != null) shortDecimalDigits = Integer.parseInt(s2);
        }
    ...
    static void setDecimalDigits(int num, boolean sf, boolean save) {
        if (sf) shortDecimalDigits = num;
        else decimalDigits = num;

        String s = "####.";
        int ct = num;
        for (; ct > 0; ct--) s += '#';
        NumberFormat nf = NumberFormat.getFormat(s);
        if (sf) shortFormat = nf;
        else showFormat = nf;
    ...
    static void setColorScale() {
        ...
        if (positiveColor == null) positiveColor = Color.green;
        if (negativeColor == null) negativeColor = Color.red;
        if (neutralColor == null) neutralColor = Color.gray;
    ...
    void reset() {
        int i;
        for (i = 0; i != getPostCount() + getInternalNodeCount(); i++) volts[i] = 0;
        curcount = 0;
    }
    ...
    void drawDots(Graphics g, Point pa, Point pb, double pos) {
        if ((!sim.simIsRunning()) || pos == 0 || !sim.dotsCheckItem.getState()) return;
        int dx = pb.x - pa.x;
        int dy = pb.y - pa.y;
    ...
    void drawPosts(Graphics g) {
        ...
        if (sim.dragElm == null && !needsHighlight()) return;
    ...
    void drawHandles(Graphics g, Color c) {
        g.setColor(c);
        if (lastHandleGrabbed == -1) g.fillRect(x - 3, y - 3, 7, 7);
        else if (lastHandleGrabbed == 0) g.fillRect(x - 4, y - 4, 9, 9);
        if (getNumHandles() > 1) {
            if (lastHandleGrabbed == -1) g.fillRect(x2 - 3, y2 - 3, 7, 7);
            else if (lastHandleGrabbed == 1) g.fillRect(x2 - 4, y2 - 4, 9, 9);
        }
    }
    ...
    */
    ...
    void drawCenteredText(Graphics g, String s, int x, int y, boolean cx) {
        ...
        if (cx) g.context.setTextAlign("center");
        g.drawString(s, x, y);
        g.restore();
    }
    ...
    void drawValues(Graphics g, String s, double hs) {
        ...
        int dpx = (int) (dpx1 * hs);
        int dpy = (int) (dpy1 * hs);
        if (dpx == 0) g.drawString(s, xc - w / 2, yc - abs(dpy) - 2);
        else {
            int xx = xc + abs(dpx) + 2;
    ...
    static String getTimeText(double v) {
        if (v >= 60) {
            ...
            v -= 60 * m;
            if (h == 0) return m + ":" + ((v >= 10) ? "" : "0") + showFormat.format(v);
    ...
    static String getUnitTextWithScale(double val, String utext, int scale, boolean fixed) {
        ...
        if (scale == SCALE_1) return nf.format(val) + " " + utext;
        if (scale == SCALE_M) return nf.format(1e3 * val) + " m" + utext;
        if (scale == SCALE_MU) return nf.format(1e6 * val) + " " + Locale.muString + utext;
    ...
    void setPowerColor(Graphics g, double w0) {
        ...
        if (i < 0) i = 0;
        if (i >= colorScaleCount) i = colorScaleCount - 1;
    ...
    double getCurrentIntoNode(int n) {
        ...
        if (n == 0 && getPostCount() == 2) return -current;
        else return current;
    }
    ...
    String getClassName() {
        return getClass().getName().replace("ru.barsukon.ecs.client.", "");
    }
    ...
    JsArrayString getInfoJS() {
        ...
        getInfo(arr);
        int i;
        for (i = 0; arr[i] != null; i++) jsarr.push(arr[i]);
        return jsarr;
    }

    double getVoltageJS(int n) {
        if (n >= volts.length) return 0;
        return volts[n];
    }

    native void addJSMethods() /*-{
        var that = this;
        this.getType = $entry(function() { return that.@ru.barsukon.ecs.client.CircuitElm::getClassName()(); });
        this.getInfo = $entry(function() { return that.@ru.barsukon.ecs.client.CircuitElm::getInfoJS()(); });
        this.getVoltageDiff = $entry(function() { return that.@ru.barsukon.ecs.client.CircuitElm::getVoltageDiff()(); });
        this.getVoltage = $entry(function(n) { return that.@ru.barsukon.ecs.client.CircuitElm::getVoltageJS(I)(n); });
        this.getCurrent = $entry(function() { return that.@ru.barsukon.ecs.client.CircuitElm::getCurrent()(); });
        this.getLabelName = $entry(function() { return that.@ru.barsukon.ecs.client.LabeledNodeElm::getName()(); });
        this.getPostCount = $entry(function() { return that.@ru.barsukon.ecs.client.CircuitElm::getPostCount()(); });
    }-*/;
