...
import ru.barsukon.ecs.client.util.PerfMonitor;
...
public class CirSim
    ...
    MenuItem aboutItem;
    MenuItem importFromLocalFileItem, importFromTextItem, exportAsUrlItem, exportAsLocalFileItem, exportAsTextItem, recoverItem, saveFileItem;
    ...
    public void setCanvasSize() {
        ...
        // 100% in ths CSS file
        if (!isMobile(sidePanelCheckboxLabel)) width = width - VERTICALPANELWIDTH;
    ...
    public void init() {
        ...
        importFromLocalFileItem = menuItemWithShortcut(
            "folder",
            "Open File...",
            Locale.LS(ctrlMetaKey + "O"),
        ...
        if (isElectron()) {
            saveFileItem = fileMenuBar.addItem(
                menuItemWithShortcut("floppy", "Save", Locale.LS(ctrlMetaKey + "S"), new MyCommand("file", "save"))
            );
            fileMenuBar.addItem(iconMenuItem("floppy", "Save As...", new MyCommand("file", "saveas")));
        } else {
            exportAsLocalFileItem = menuItemWithShortcut(
                "floppy",
                "Save As...",
        ...
        if (VERTICALPANELWIDTH > 166) VERTICALPANELWIDTH = 166;
        ...
        m.addSeparator();
        m.addItem(
            selectAllItem = menuItemWithShortcut(
                "select-all",
                "Select All",
                Locale.LS(ctrlMetaKey + "A"),
        ...
        m.addItem(
            crossHairCheckItem = new CheckboxMenuItem(
                Locale.LS("Show Cursor Cross Hairs"),
                new Command() {
                    public void execute() {
        ...
        euroResistorCheckItem = new CheckboxMenuItem(
            Locale.LS("European Resistors"),
            new Command() {
                public void execute() {
        ...
        euroGatesCheckItem = new CheckboxMenuItem(
            Locale.LS("IEC Gates"),
            new Command() {
                public void execute() {
                    setOptionInStorage("euroGates", euroGatesCheckItem.getState());
                    int i;
                    for (i = 0; i != elmList.size(); i++) getElm(i).setPoints();
                }
            }
        );
        euroGatesCheckItem.setState(euroGates);
        printableCheckItem = new CheckboxMenuItem(
            Locale.LS("White Background"),
            new Command() {
                public void execute() {
                    int i;
                    for (i = 0; i < scopeCount; i++) scopes[i].setRect(scopes[i].rect);
        ...
        conventionCheckItem = new CheckboxMenuItem(
            Locale.LS("Conventional Current Motion"),
            new Command() {
                public void execute() {
        ...
        m.addItem(
            mouseWheelEditCheckItem = new CheckboxMenuItem(
                Locale.LS("Edit Values With Mouse Wheel"),
                new Command() {
                    public void execute() {
        ...
        menuBar
            ...
            .setAttribute("onclick", "document.getElementsByClassName('toptrigger')[0].checked = false");
        ...
        elmMenuBar.addItem(
            elmFloatScopeMenuItem = new MenuItem(Locale.LS("View in New Undocked Scope"), new MyCommand("elm", "viewInFloatScope"))
        );
        ...
        elmMenuBar.addItem(
            elmSplitMenuItem = menuItemWithShortcut("", "Split Wire", Locale.LS(ctrlMetaKey + "click"), new MyCommand("elm", "split"))
        );
        ...
        Window.addWindowClosingHandler(
            new Window.ClosingHandler() {
                public void onWindowClosing(ClosingEvent event) {
                    ...
                    // warning is given
                    if (unsavedChanges && !isElectron()) event.setMessage(Locale.LS("Are you sure?  There are unsaved changes."));
                }
            }
        );
        setupJSInterface();
    ...
    void setColors(String positiveColor, String negativeColor, String neutralColor, String selectColor, String currentColor) {
        ...
        if (positiveColor != null) CircuitElm.positiveColor = new Color(URL.decodeQueryString(positiveColor));
        else if (getOptionFromStorage("alternativeColor", false)) CircuitElm.positiveColor = Color.blue;

        if (negativeColor != null) CircuitElm.negativeColor = new Color(URL.decodeQueryString(negativeColor));
    ...
    MenuItem menuItemWithShortcut(String icon, String text, String shortcut, MyCommand cmd) {
        final String edithtml = "<div style=\"white-space:nowrap\"><div style=\"display:inline-block;width:100%;\"><i class=\"ecs-";
        String nbsp = "&nbsp;";
        if (icon == "") nbsp = "";
    ...
    MenuItem iconMenuItem(String icon, String text, Command cmd) {
        String icoStr = "<i class=\"ecs-" + icon + "\"></i>&nbsp;" + Locale.LS(text); // <i
        // class="ecs-"></i>&nbsp;
    ...
    void loadShortcuts() {
        ...
        int i;
        for (i = 0; i != shortcuts.length; i++) shortcuts[i] = null;
    ...
    static native void doTouchHandlers(CirSim sim, CanvasElement cv) /*-{
	...
	cv.addEventListener("touchstart", function (e) {
  		...
  		if (e.timeStamp-lastTap < 300) {
  		    ...
  		    tmout = setTimeout(function() {
  		        sim.@ru.barsukon.ecs.client.CirSim::longPress()();
  		    }, 500);
  		}
  		lastTap = e.timeStamp;
  		...
  		if (e.touches.length > 1)
  		    sim.@ru.barsukon.ecs.client.CirSim::twoFingerTouch(II)(mouseEvent.clientX, mouseEvent.clientY - cv.getBoundingClientRect().y);
	}, false);
  		...
  		var touch2 = e.touches[e.touches.length-1];
	        if (e.touches.length > 1) {
  		    var newScale = Math.hypot(touch1.clientX-touch2.clientX, touch1.clientY-touch2.clientY);
	            sim.@ru.barsukon.ecs.client.CirSim::zoomCircuit(D)(40*(Math.log(newScale)-Math.log(lastScale)));
	            lastScale = newScale;
	        }
    ...
    public void composeMainMenu(MenuBar mainMenuBar, int num) {
        ...
        infoMenuBar.addItem(getClassCheckItem(Locale.LS("Add Labeled Node"), "LabeledNodeElm"));
        mainMenuBar.addItem(
            SafeHtmlUtils.fromTrustedString(CheckboxMenuItem.checkBoxHtml + Locale.LS("&nbsp;</div>Informational")),
            infoMenuBar
        );
    ...
    public void composeSelectScopeMenu(MenuBar sb) {
        ...
        for (int i = 0; i < scopeCount; i++) {
            ...
            if (l != "") s += " (" + SafeHtmlUtils.htmlEscape(l) + ")";
        ...
        for (MenuItem mi : selectScopeMenuItems) sb.addItem(mi);
    }
    ...
    CheckboxMenuItem getClassCheckItem(String s, String t) {
        ...
        if (elm != null) {
            if (elm.needsShortcut()) {
                ...
                if (shortcuts[elm.getShortcut()] != null && !shortcuts[elm.getShortcut()].equals(t)) console(
                    "already have shortcut for " + (char) elm.getShortcut() + " " + elm
                );
    ...
    void centreCircuit() {
        if (
            elmList == null
        ) return; // avoid exception if called during initialization
    ...
    public void updateCircuit() {
        ...
        // Run circuit
        if (simRunning) {
            if (needsStamp) console("needsStamp while simRunning?");
        ...
        if (mouseMode != CirSim.MODE_DRAG_ROW && mouseMode != CirSim.MODE_DRAG_COLUMN) {
            for (int i = 0; i != postDrawList.size(); i++) CircuitElm.drawPost(g, postDrawList.get(i));
        }
    ...
    void drawBottomArea(Graphics g) {
        ...
        for (i = 0; i != ct; i++) scopes[i].selectScope(mouseCursorX, mouseCursorY);
        if (scopeElmArr != null) for (i = 0; i != scopeElmArr.length; i++) scopeElmArr[i].selectScope(mouseCursorX, mouseCursorY);
        for (i = 0; i != ct; i++) scopes[i].draw(g);
        ...
        if (stopMessage != null) {
            ...
            if (mouseElm != null) {
                if (mousePost == -1) {
                    ...
                    if (info[1] != null) info[1] = Locale.LS(info[1]);
                } else info[0] = "V = " + CircuitElm.getUnitText(mouseElm.getPostVoltage(mousePost), "V");
                // /* //shownodes
                ...
                if (timerate >= .1) info[0] += " (" + CircuitElm.showFormat.format(timerate) + "x)";
            ...
            if (badnodes > 0) info[i++] = badnodes + ((badnodes == 1) ? Locale.LS(" bad connection") : Locale.LS(" bad connections"));
            if (savedFlag) info[i++] = "(saved)";
    ...
    Color getBackgroundColor() {
        if (printableCheckItem.getState()) return Color.white;
        return Color.black;
    }
    ...
    boolean scopeMenuIsSelected(Scope s) {
        if (scopeMenuSelected < 0) return false;
        if (scopeMenuSelected < scopeCount) return scopes[scopeMenuSelected] == s;
    ...
    void setupScopes() {
        ...
        for (i = 0; i < scopeCount; i++) {
            if (scopes[i].needToRemove()) {
                int j;
                for (j = i; j != scopeCount; j++) scopes[j] = scopes[j + 1];
                scopeCount--;
                i--;
                continue;
            }
        ...
        while (scopeCount > 0 && scopes[scopeCount - 1].getElm() == null) scopeCount--;
        int h = canvasHeight - circuitArea.height;
        pos = 0;
        for (i = 0; i != scopeCount; i++) scopeColCount[i] = 0;
        for (i = 0; i != scopeCount; i++) {
    ...
    String getHint() {
        ...
        if (hintType == HINT_LC) {
            ...
            return Locale.LS("res.f = ") + CircuitElm.getUnitText(1 / (2 * pi * Math.sqrt(ie.inductance * ce.capacitance)), "Hz");
        }
        if (hintType == HINT_RC) {
        ...
        for (i = 0; i != elmList.size(); i++) {
            ...
            if (cn != null && cn2 != null) {
                ...
                for (Map.Entry<Point, NodeMapEntry> entry : nodeMap.entrySet()) {
                    if (entry.getValue() == cn2) entry.setValue(cn);
                }
                // mergeCount++;
                continue;
            }
        ...
        for (i = 0; i != wireInfoList.size(); i++) {
            ...
            for (j = 0; j != cn1.links.size(); j++) {
                ...
                if (pt.x == wire.x && pt.y == wire.y) {
                    ...
                    wire instanceof LabeledNodeElm &&
    ...
    void setGroundNode() {
        ...
        if (!gotGround && volt != null && !gotRail) {
            ...
            if (cln != null) cln.node = 0;
            else nodeMap.put(pt, new NodeMapEntry(0));
        } else {
        ...
        for (i = 0; i != elmList.size(); i++) {
            ...
            for (j = 0; j != posts; j++) {
                ...
                if (cln == null || cln.node == -1) {
                    ...
                    if (cln != null) cln.node = nodeList.size();
                    else nodeMap.put(pt, new NodeMapEntry(nodeList.size()));
                    nodeList.addElement(cn);
                } else {
                    int n = cln.node;
                    CircuitNodeLink cnl = new CircuitNodeLink();
        ...
        while (changed) {
            ...
            for (i = 0; i != elmList.size(); i++) {
                ...
                for (j = 0; j < ce.getConnectionNodeCount(); j++) {
                    ...
                    if (hg) hasGround = true;
                    if (!closure[ce.getConnectionNode(j)]) {
                        if (hg) closure[ce.getConnectionNode(j)] = changed = true;
                        continue;
                    }
                    int k;
                ...
                if (hasGround) nodesWithGroundConnection.add(ce);
            }
            if (changed) continue;
    ...
    void analyzeCircuit() {
        ...
        for (i = 0; i != elmList.size(); i++) {
            CircuitElm ce = getElm(i);
            if (ce instanceof VoltageElm) {
                if (gotVoltageSource) showResistanceInVoltageSources = false;
                else gotVoltageSource = true;
            }
        }
    ...
    void stampCircuit() {
        ...
        if (lastNodeVoltages == null || lastNodeVoltages.length != nodeVoltages.length) lastNodeVoltages = new double[nodeList.size() - 1];
        origMatrix = new double[matrixSize][matrixSize];
        origRightSide = new double[matrixSize];
        circuitMatrixSize = circuitMatrixFullSize = matrixSize;
        circuitRowInfo = new RowInfo[matrixSize];
        circuitPermute = new int[matrixSize];
        for (i = 0; i != matrixSize; i++) circuitRowInfo[i] = new RowInfo();
        ...
        for (i = 0; i != elmList.size(); i++) {
            ...
            if (elmArr[i] instanceof ScopeElm) scopeElmCount++;
        }
        ...
        int j = 0;
        for (i = 0; i != elmList.size(); i++) {
            if (elmArr[i] instanceof ScopeElm) scopeElmArr[j++] = (ScopeElm) elmArr[i];
        }
    ...
    boolean simplifyMatrix(int matrixSize) {
        ...
        for (i = 0; i != matrixSize; i++) {
            ...
            if (j == matrixSize) {
                ...
                for (j = 0; j != i; j++) if (circuitMatrix[j][qp] != 0) break;
        ...
        for (i = 0; i != matrixSize; i++) {
            ...
            for (j = 0; j != matrixSize; j++) {
                ...
                else newmatx[ii][ri.mapCol] += circuitMatrix[i][j];
            }
            ii++;
        }
        ...
        for (i = 0; i != matrixSize; i++) origRightSide[i] = circuitRightSide[i];
        for (i = 0; i != matrixSize; i++) for (j = 0; j != matrixSize; j++) origMatrix[i][j] = circuitMatrix[i][j];
    ...
    void makePostDrawList() {
        ...
        for (Map.Entry<Point, Integer> entry : postCountMap.entrySet()) {
            if (entry.getValue() != 2) postDrawList.add(entry.getKey());
            ...
            if (entry.getValue() == 1) {
                ...
                for (j = 0; j != elmList.size() && !bad; j++) {
                    ...
                    for (k = 0; k != pc; k++) if (ce.getPost(k).equals(cn)) break;
                    if (k == pc) bad = true;
                }
                if (bad) badConnectionList.add(cn);
            }
        }
        postCountMap = null;
    }
    ...
    class FindPathInfo {
        ...
        boolean findPath(int n1) {
            ...
            for (i = 0; i != cn.links.size(); i++) {
                ...
                CircuitElm ce = cnl.elm;
                if (checkElm(n1, ce)) return true;
            }
            if (n1 == 0) {
                for (i = 0; i != nodesWithGroundConnection.size(); i++) if (checkElm(0, nodesWithGroundConnection.get(i))) return true;
            }
            return false;
        }

        boolean checkElm(int n1, CircuitElm ce) {
            if (ce == firstElm) return false;
            if (type == INDUCT) {
                // inductors need a path free of current sources
                if (ce instanceof CurrentElm) return false;
            }
            if (type == VOLTAGE) {
                // when checking for voltage loops, we only care about voltage
                // sources/wires/ground
                if (!(ce.isWireEquivalent() || ce instanceof VoltageElm || ce instanceof GroundElm)) return false;
            }
            // when checking for shorts, just check wires
            if (type == SHORT && !ce.isWireEquivalent()) return false;
            if (type == CAP_V) {
                // checking for capacitor/voltage source loops
                if (!(ce.isWireEquivalent() || ce instanceof CapacitorElm || ce instanceof VoltageElm)) return false;
            }
            if (n1 == 0) {
                // look for posts which have a ground connection;
                // our path can go through ground
                int j;
                for (j = 0; j != ce.getConnectionNodeCount(); j++) if (
                    ce.hasGroundConnection(j) && findPath(ce.getConnectionNode(j))
                ) return true;
            }
            int j;
            for (j = 0; j != ce.getConnectionNodeCount(); j++) {
                if (ce.getConnectionNode(j) == n1) {
                    if (ce.hasGroundConnection(j) && findPath(0)) return true;
    ...
    void stampMatrix(int i, int j, double x) {
        if (Double.isInfinite(x)) debugger();
        if (i > 0 && j > 0) {
            if (circuitNeedsMap) {
    ...
    boolean canDelayWireProcessing() {
        int i;
        for (i = 0; i != scopeCount; i++) if (scopes[i].viewingWire()) return false;
        for (i = 0; i != elmList.size(); i++) if (
            getElm(i) instanceof ScopeElm && ((ScopeElm) getElm(i)).elmScope.viewingWire()
        ) return false;
        return true;
    }
    ...
    void runCircuit(boolean didAnalyze) {
        ...
        // consistent.
        if (1000 >= steprate * (tm - lastIterTime) && !didAnalyze) return;
        ...
        for (iter = 1;; iter++) {
            ...
            int i, j, subiter;
            for (i = 0; i != elmArr.length; i++) elmArr[i].startIteration();
            steps++;
            ...
            for (subiter = 0; subiter != subiterCount; subiter++) {
                ...
                // converged = false;
                for (i = 0; i != circuitMatrixSize; i++) circuitRightSide[i] = origRightSide[i];
                if (circuitNonLinear) {
                    for (i = 0; i != circuitMatrixSize; i++) for (j = 0; j != circuitMatrixSize; j++) circuitMatrix[i][j] =
                        origMatrix[i][j];
                }
                for (i = 0; i != elmArr.length; i++) elmArr[i].doStep();
                if (stopMessage != null) return;
                ...
                if (printit) {
                    for (j = 0; j != circuitMatrixSize; j++) {
                        String x = "";
                        for (i = 0; i != circuitMatrixSize; i++) x += circuitMatrix[j][i] + ",";
                        x += "\n";
                        console(x);
                    }
                    console("done");
            ...
            if (subiter > 5 || timeStep < maxTimeStep) console("converged after " + subiter + " iterations, timeStep = " + timeStep);
            ...
            for (i = 0; i != elmArr.length; i++) elmArr[i].stepFinished();
            if (!delayWireProcessing) calcWireCurrents();
            for (i = 0; i != scopeCount; i++) scopes[i].timeStep();
            for (i = 0; i != scopeElmArr.length; i++) scopeElmArr[i].stepScope();
            callTimeStepHook();
            // save last node voltages so we can restart the next iteration if necessary
            for (i = 0; i != lastNodeVoltages.length; i++) lastNodeVoltages[i] = nodeVoltages[i];
            ...
            // (20fps) by default
            if (
                (timeStepCount - timeStepCountAtFrameStart) * 1000 >= steprate * (tm - lastIterTime) ||
    ...
    void calcWireCurrents() {
        ...
        for (i = 0; i != wireInfoList.size(); i++) {
            ...
            if (wi.post == 0 || (wi.wire instanceof LabeledNodeElm)) wi.wire.setCurrent(-1, cur);
            else wi.wire.setCurrent(-1, -cur);
        }
    }
    ...
    public void resetAction() {
        ...
        t = timeStepAccum = 0;
        timeStepCount = 0;
        for (i = 0; i != elmList.size(); i++) getElm(i).reset();
        for (i = 0; i != scopeCount; i++) scopes[i].resetGraph(true);
        repaint();
    }
    ...
    static native void electronSaveAs(String dump) /*-{
        $wnd.showSaveDialog().then(function (file) {
            ...
            @ru.barsukon.ecs.client.CirSim::electronSaveAsCallback(Ljava/lang/String;)(file.filePath.toString());
        });
    }-*/;

    static native void electronSave(String dump) /*-{
        $wnd.saveFile(null, dump);
        @ru.barsukon.ecs.client.CirSim::electronSaveCallback()();
    }-*/;
    ...
    static native void electronOpenFile() /*-{
        $wnd.openFile(function (text, name) {
            @ru.barsukon.ecs.client.CirSim::electronOpenFileCallback(Ljava/lang/String;Ljava/lang/String;)(text, name);
        });
    }-*/;
    ...
    public void menuPerformed(String menu, String item) {
        ...
        if (item == "about") aboutBox = new AboutBox(ElectronicCircuitSimulator.versionString);
        ...
        if (item == "dcanalysis") doDCAnalysis();
        ...
        if (item == "stackAll") stackAll();
        if (item == "unstackAll") unstackAll();
        if (item == "combineAll") combineAll();
        if (item == "separateAll") separateAll();
        ...
        if (item == "viewInScope" && menuElm != null) {
            int i;
            for (i = 0; i != scopeCount; i++) if (scopes[i].getElm() == null) break;
            if (i == scopeCount) {
        ...
        if (item.startsWith("addToScope") && menuElm != null) {
            ...
            if (n < scopeCount + countScopeElms()) {
                if (n < scopeCount) scopes[n].addElm(menuElm);
                else getNthScopeElm(n - scopeCount).elmScope.addElm(menuElm);
            }
            scopeMenuSelected = -1;
        }
        ...
        if (menu == "scopepop") {
            pushUndo();
            Scope s;
            if (menuScope != -1) s = scopes[menuScope];
            else s = ((ScopeElm) mouseElm).elmScope;
            ...
            if (item == "undock") {
                ...
                for (i = menuScope; i < scopeCount; i++) scopes[i] = scopes[i + 1];
                scopeCount--;

                needAnalyze(); // need to rebuild scopeElmArr
            }
            if (item == "remove") s.setElm(null); // setupScopes() will clean this up
            if (item == "removeplot") s.removePlot(menuPlot);
            if (item == "speed2") s.speedUp();
            if (item == "speed1/2") s.slowDown();
            // if (item=="scale")
            // scopes[menuScope].adjustScale(.5);
            if (item == "maxscale") s.maxScale();
            if (item == "stack") stackScope(menuScope);
            if (item == "unstack") unstackScope(menuScope);
            if (item == "combine") combineScope(menuScope);
            if (item == "selecty") s.selectY();
            if (item == "reset") s.resetGraph(true);
            if (item == "properties") s.properties();
    ...
    int countScopeElms() {
        int c = 0;
        for (int i = 0; i != elmList.size(); i++) {
            if (elmList.get(i) instanceof ScopeElm) c++;
        }
        return c;
    }
    ...
    ScopeElm getNthScopeElm(int n) {
        for (int i = 0; i != elmList.size(); i++) {
            if (elmList.get(i) instanceof ScopeElm) {
                n--;
                if (n < 0) return (ScopeElm) elmList.get(i);
            }
        }
    ...
    boolean canStackScope(int s) {
        ...
        if (s == 0) s = 1;
        if (scopes[s].position == scopes[s - 1].position) return false;
        return true;
    }
    ...
    boolean canUnstackScope(int s) {
        ...
        if (scopes[s].position != scopes[s - 1].position) {
            if (
                s + 1 < scopeCount && scopes[s + 1].position == scopes[s].position
            ) // the top scope in the stack // Allow you to unstack by selecting
            return true;
            else return false;
        }
        return true;
    ...
    void stackScope(int s) {
        if (!canStackScope(s)) return;
        if (s == 0) {
            s = 1;
        }
        scopes[s].position = scopes[s - 1].position;
        for (s++; s < scopeCount; s++) scopes[s].position--;
    }

    void unstackScope(int s) {
        if (!canUnstackScope(s)) return;
        if (s == 0) {
            s = 1;
        }
        if (
            scopes[s].position != scopes[s - 1].position
        ) // stack // Allow you to unstack by selecting the top scope in the
        s++;
        for (; s < scopeCount; s++) scopes[s].position++;
    }

    void combineScope(int s) {
        if (!canCombineScope(s)) return;
        if (s == 0) {
            s = 1;
        }
    ...
    void separateAll() {
        ...
        int ct = 0;
        for (i = 0; i < scopeCount; i++) ct = scopes[i].separate(newscopes, ct);
        scopes = newscopes;
        scopeCount = ct;
    }
    ...
    void readCircuit(byte b[], int flags) {
        ...
        if ((flags & RC_SUBCIRCUITS) != 0) updateModels();
    }
    ...
    int locateElm(CircuitElm elm) {
        int i;
        for (i = 0; i != elmList.size(); i++) if (elm == elmList.elementAt(i)) return i;
        return -1;
    }
    ...
    boolean onlyGraphicsElmsSelected() {
        if (mouseElm != null && !(mouseElm instanceof GraphicElm)) return false;
        int i;
        for (i = 0; i != elmList.size(); i++) {
            CircuitElm ce = getElm(i);
            if (ce.isSelected() && !(ce instanceof GraphicElm)) return false;
        }
        return true;
    }
    ...
    void dragPost(int x, int y, boolean all) {
        if (draggingPost == -1) {
            draggingPost = (Graphics.distanceSq(mouseElm.x, mouseElm.y, x, y) > Graphics.distanceSq(mouseElm.x2, mouseElm.y2, x, y))
                ? 1
                : 0;
        }
        int dx = x - dragGridX;
    ...
    void setMouseElm(CircuitElm ce) {
        if (ce != mouseElm) {
            ...
            mouseElm = ce;
            int i;
            for (i = 0; i < adjustables.size(); i++) adjustables.get(i).setMouseElm(ce);
        }
    }
    ...
    void doPopupMenu() {
        ...
        if (scopeSelected != -1) {
            if (scopes[scopeSelected].canMenu()) {
                ...
                scopePopupMenu.doScopePopupChecks(
                    false,
                    canStackScope(scopeSelected),
                    canCombineScope(scopeSelected),
                    canUnstackScope(scopeSelected),
                    scopes[scopeSelected]
                );
    ...
    public void onClick(ClickEvent e) {
        ...
        // clearSelection();
        // }
        if ((e.getNativeButton() == NativeEvent.BUTTON_MIDDLE)) scrollValues(
    ...
    public void onDoubleClick(DoubleClickEvent e) {
        ...
        if (mouseElm != null && !(mouseElm instanceof SwitchElm) && !noEditCheckItem.getState()) doEdit(mouseElm);
    }
    ...
    public void onMouseDown(MouseDownEvent e) {
        ...
        if (
            !(dialogIsShowing()) &&
            ((scopeSelected != -1 && scopes[scopeSelected].cursorInSettingsWheel()) ||
            ...
            Scope s;
            if (scopeSelected != -1) s = scopes[scopeSelected];
            else s = ((ScopeElm) mouseElm).elmScope;
            s.properties();
            clearSelection();
            mouseDragging = false;
    ...
    public void onMouseWheel(MouseWheelEvent e) {
        ...
        else if (scopeSelected != -1 && !zoomOnly) scopes[scopeSelected].onMouseWheel(e);
    ...
    void scrollValues(int x, int y, int deltay) {
        if (mouseElm != null && !dialogIsShowing() && scopeSelected == -1) if (
            mouseElm instanceof ResistorElm || mouseElm instanceof CapacitorElm || mouseElm instanceof InductorElm
        ) {
    ...
    String copyOfSelectedElms() {
        ...
        for (int i = elmList.size() - 1; i >= 0; i--) {
            ...
            if (ce.isSelected() && !(ce instanceof ScopeElm)) r += ce.dump() + "\n";
        }
        return r;
    }
    ...
    boolean anySelectedButMouse() {
        for (int i = 0; i != elmList.size(); i++) if (getElm(i) != mouseElm && getElm(i).selected) return true;
        return false;
    }
    ...
    public static CircuitElm constructElement(String n, int x1, int y1) {
        if (n == "GroundElm") return (CircuitElm) new GroundElm(x1, y1);
        if (n == "ResistorElm") return (CircuitElm) new ResistorElm(x1, y1);
        if (n == "RailElm") return (CircuitElm) new RailElm(x1, y1);
        if (n == "SwitchElm") return (CircuitElm) new SwitchElm(x1, y1);
        if (n == "Switch2Elm") return (CircuitElm) new Switch2Elm(x1, y1);
        if (n == "WireElm") return (CircuitElm) new WireElm(x1, y1);
        if (n == "CapacitorElm") return (CircuitElm) new CapacitorElm(x1, y1);
        if (n == "InductorElm") return (CircuitElm) new InductorElm(x1, y1);
        if (n == "DCVoltageElm" || n == "VoltageElm") return (CircuitElm) new DCVoltageElm(x1, y1);
        if (n == "VarRailElm") return (CircuitElm) new VarRailElm(x1, y1);
        if (n == "PotElm") return (CircuitElm) new PotElm(x1, y1);
        if (n == "OutputElm") return (CircuitElm) new OutputElm(x1, y1);
        if (n == "CurrentElm") return (CircuitElm) new CurrentElm(x1, y1);
        if (n == "ProbeElm") return (CircuitElm) new ProbeElm(x1, y1);
        if (n == "DiodeElm") return (CircuitElm) new DiodeElm(x1, y1);
        if (n == "ACVoltageElm") return (CircuitElm) new ACVoltageElm(x1, y1);
        if (n == "ACRailElm") return (CircuitElm) new ACRailElm(x1, y1);
        if (n == "SweepElm") return (CircuitElm) new SweepElm(x1, y1);
        if (n == "LampElm") return (CircuitElm) new LampElm(x1, y1);
        if (n == "PushSwitchElm") return (CircuitElm) new PushSwitchElm(x1, y1);
        if (n == "BoxElm") return (CircuitElm) new BoxElm(x1, y1);
        if (n == "LineElm") return (CircuitElm) new LineElm(x1, y1);
        if (n == "TextElm") return (CircuitElm) new TextElm(x1, y1);
        if (n == "LabeledNodeElm") return (CircuitElm) new LabeledNodeElm(x1, y1);
        if (n == "AmmeterElm") return new AmmeterElm(x1, y1);
        if (n == "OhmMeterElm") return (CircuitElm) new OhmMeterElm(x1, y1);
        if (n == "ScopeElm") return (CircuitElm) new ScopeElm(x1, y1);
        if (n == "FuseElm") return (CircuitElm) new FuseElm(x1, y1);
        if (n == "WattmeterElm") return (CircuitElm) new WattmeterElm(x1, y1);
    ...
    public void updateModels() {
        int i;
        for (i = 0; i != elmList.size(); i++) elmList.get(i).updateModels();
    }
    ...
    void dumpNodelist() {
        ...
        for (i = 0; i < elmList.size(); i++) {
            ...
            cs = cs.substring(p + 1);
            if (cs == "WireElm") continue;
            if (cs == "LabeledNodeElm") cs = cs + " " + ((LabeledNodeElm) e).text;
            s = cs;
    ...
    boolean isSelection() {
        for (int i = 0; i != elmList.size(); i++) if (getElm(i).isSelected()) return true;
        return false;
    }
    ...
    public CustomCompositeModel getCircuitAsComposite() {
        ...
        for (i = 0; i != elmList.size(); i++) {
            ...
            if (ce instanceof LabeledNodeElm) {
                ...
                if (extnodes[ce.getNode(0)]) continue;
        ...
        for (i = 0; i != elmList.size(); i++) {
            ...
            if (ce instanceof WireElm || ce instanceof LabeledNodeElm || ce instanceof ScopeElm) continue;
            if (ce instanceof GraphicElm || ce instanceof GroundElm) continue;
            int j;
            if (nodeDump.length() > 0) nodeDump += "\r";
    ...
    static void invertMatrix(double a[][], int n) {
        ...
        for (i = 0; i != n; i++) {
            ...
            b[i] = 1;
            lu_solve(a, n, ipvt, b);
            for (j = 0; j != n; j++) inva[j][i] = b[j];
        }

        // return in original matrix
        for (i = 0; i != n; i++) for (j = 0; j != n; j++) a[i][j] = inva[i][j];
    }
    ...
    native void setupJSInterface() /*-{
	    var that = this;
	    $wnd.ECS = {
	        setSimRunning: $entry(function(run) { that.@ru.barsukon.ecs.client.CirSim::setSimRunning(Z)(run); } ),
	        getTime: $entry(function() { return that.@ru.barsukon.ecs.client.CirSim::t; } ),
	        ...
	        getElements: $entry(function() { return that.@ru.barsukon.ecs.client.CirSim::getJSElements()(); } ),
	        exportCircuit: $entry(function() { return that.@ru.barsukon.ecs.client.CirSim::dumpCircuit()(); } ),
	        importCircuit: $entry(function(circuit, subcircuitsOnly) { return that.@ru.barsukon.ecs.client.CirSim::importCircuitFromText(Ljava/lang/String;Z)(circuit, subcircuitsOnly); })
	    };
    ...
    native void callUpdateHook() /*-{
	    var hook = $wnd.ECS.onupdate;
	    if (hook)
	    	hook($wnd.ECS);
	}-*/;

    native void callAnalyzeHook() /*-{
            var hook = $wnd.ECS.onanalyze;
            if (hook)
                hook($wnd.ECS);
    	}-*/;

    native void callTimeStepHook() /*-{
	    var hook = $wnd.ECS.ontimestep;
	    if (hook)
	    	hook($wnd.ECS);
	}-*/;
