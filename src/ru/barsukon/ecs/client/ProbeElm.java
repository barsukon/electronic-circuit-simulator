...
class ProbeElm extends CircuitElm {
    ...
    void draw(Graphics g) {
        ...
        if (drawAsCircle()) len = circleSize * 2;
        calcLeads((int) len);
        ...
        g.setFont(f);
        if (this == sim.plotXElm) drawCenteredText(g, "X", center.x, center.y, true);
        if (this == sim.plotYElm) drawCenteredText(g, "Y", center.x, center.y, true);
        if (mustShowVoltage()) {
            String s = "";
            switch (meter) {
                case TP_VOL:
        ...
        if (y2 > y) plusPoint.y += 4;
        if (y > y2) plusPoint.y += 3;
    ...
    public void setEditValue(int n, EditInfo ei) {
        if (n == 0) {
            if (ei.checkbox.getState()) flags = FLAG_SHOWVOLTAGE;
            else flags &= ~FLAG_SHOWVOLTAGE;
        }
        if (n == 1) {
            meter = ei.choice.getSelectedIndex();
        }
        if (n == 2) scale = ei.choice.getSelectedIndex();
        if (n == 3) flags = ei.changeFlag(flags, FLAG_CIRCLE);
    }
}
