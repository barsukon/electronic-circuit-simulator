...
class Diode {
    ...
    void doStep(double voltdiff) {
        ...
        if (Math.abs(voltdiff - lastvoltdiff) > .01) sim.converged = false;
        ...
        if (voltdiff >= 0 || zvoltage == 0) {
            ...
            double nc = leakage * (Math.exp(voltdiff * vdcoef) - Math.exp((-voltdiff - zoffset) * vzcoef) - 1) + geq * (-voltdiff);
    ...
    double calculateCurrent(double voltdiff) {
        if (voltdiff >= 0 || zvoltage == 0) return leakage * (Math.exp(voltdiff * vdcoef) - 1);
