...
public class Graphics {
    ...
    public void drawPolyline(int[] xpoints, int[] ypoints, int n) {
        ...
        context.beginPath();
        for (i = 0; i < n; i++) {
            if (i == 0) context.moveTo(xpoints[i], ypoints[i]);
            else context.lineTo(xpoints[i], ypoints[i]);
        }
        context.closePath();
        context.stroke();
    }
    ...
    public void fillPolygon(Polygon p) {
        ...
        for (i = 0; i < p.npoints; i++) {
            if (i == 0) context.moveTo(p.xpoints[i], p.ypoints[i]);
            else context.lineTo(p.xpoints[i], p.ypoints[i]);
        }
        context.closePath();
        context.fill();
    }
    ...
    static native void setLineDash(Context2d context, int a, int b) /*-{
	       if (a == 0)
	           context.setLineDash([]);
	       else
    ...
    private static native void requestExitFullScreen() /*-{
	   var d = $doc;
