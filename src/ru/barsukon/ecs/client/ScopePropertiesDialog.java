...
public class ScopePropertiesDialog extends Dialog implements ValueChangeHandler<Boolean> {
    ...
    class PlotClickHandler implements ClickHandler {
        ...
        public void onClick(ClickEvent event) {
            ...
            for (int i = 0; i < chanButtons.size(); i++) {
                if (i == num) chanButtons.get(i).addStyleName("chsel");
                else chanButtons.get(i).removeStyleName("chsel");
            }
            updateUi();
        }
    }
    ...
    class downClickHandler implements ClickHandler {
        ...
        public void onClick(ClickEvent event) {
            double lasts, s;
            if (!scope.isManualScale() || plotSelection > scope.visiblePlots.size()) return;
    ...
    void updateChannelButtons() {
        if (plotSelection >= scope.visiblePlots.size()) plotSelection = 0;
        ...
        for (int i = 0; i < scope.visiblePlots.size(); i++) {
            if (i >= chanButtons.size()) {
                ...
                if (CircuitElm.whiteColor == Color.white) b.addStyleName("chbut-black");
                else b.addStyleName("chbut-white");
                channelButtonsp.add(b);
            }
            Button b = chanButtons.get(i);
            b.setHTML(getChannelButtonLabel(i));
            if (i == plotSelection) b.addStyleName("chsel");
            else b.removeStyleName("chsel");
        }
    }
    ...
    public ScopePropertiesDialog(CirSim asim, Scope s) {
        ...
        autoButton.addValueChangeHandler(
        ...
        maxButton.addValueChangeHandler(
            new ValueChangeHandler<Boolean>() {
                public void onValueChange(ValueChangeEvent<Boolean> e) {
                    scope.setManualScale(false, false);
        ...
        manualButton.addValueChangeHandler(
            new ValueChangeHandler<Boolean>() {
                public void onValueChange(ValueChangeEvent<Boolean> e) {
        ...
        dcButton.addValueChangeHandler(
            new ValueChangeHandler<Boolean>() {
                public void onValueChange(ValueChangeEvent<Boolean> e) {
                    if (plotSelection < scope.visiblePlots.size()) scope.visiblePlots.get(plotSelection).setAcCoupled(false);
                    updateUi();
                }
            }
        );
        ...
        acButton.addValueChangeHandler(
            new ValueChangeHandler<Boolean>() {
                public void onValueChange(ValueChangeEvent<Boolean> e) {
                    if (plotSelection < scope.visiblePlots.size()) scope.visiblePlots.get(plotSelection).setAcCoupled(true);
                    updateUi();
                }
            }
        );
        ...
        resetPosButton.addClickHandler(
            new ClickHandler() {
                public void onClick(ClickEvent event) {
        ...
        if (labelText != null) labelTextBox.setText(labelText);
        addItemToGrid(grid, applyButton2 = new Button(Locale.LS("Apply")));
        applyButton2.addClickHandler(
            new ClickHandler() {
                public void onClick(ClickEvent event) {
                    apply();
                }
        ...
        saveAsDefaultButton.addClickHandler(
            new ClickHandler() {
                public void onClick(ClickEvent event) {
                    scope.saveAsDefault();
                }
    ...
    class labelledGridManager {
        ...
        void updateRowVisibility() {
            for (int i = 0; i < labels.size(); i++) {
                ...
                if (i < labels.size() - 1) end = labelRows.get(i + 1);
                else end = g.getRowCount();
                for (int j = start + 1; j < end; j++) g.getRowFormatter().setVisible(j, labels.get(i).expanded);
            }
        }
    }
    ...
    void refreshDraw() {
        ...
        if (!scope.isManualScale()) updateManualScaleUi();
    }
    ...
    double getManualScaleValue() {
        try {
            ...
            if (d < Scope.MIN_MAN_SCALE) d = Scope.MIN_MAN_SCALE;
            return d;
        } catch (Exception e) {
            return 0;
        }
    ...
    void apply() {
        ...
        if (scope.isManualScale()) {
            double d = getManualScaleValue();
            if (d > 0) scope.setManualScaleValue(plotSelection, d);
        }
    }
