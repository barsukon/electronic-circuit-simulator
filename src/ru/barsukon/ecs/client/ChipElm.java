...
abstract class ChipElm extends CircuitElm {
    ...
    public ChipElm(int xx, int yy) {
        super(xx, yy);
        if (needsBits()) bits = defaultBitCount();
        highVoltage = 5;
        noDiagonal = true;
        setupPins();
    ...
    public ChipElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
        super(xa, ya, xb, yb, f);
        if (needsBits()) if (st.hasMoreTokens()) bits = new Integer(st.nextToken()).intValue();
        else bits = defaultBitCount();
        ...
        for (i = 0; i != getPostCount(); i++) {
            if (pins == null) volts[i] = new Double(st.nextToken()).doubleValue();
    ...
    void drawChip(Graphics g) {
        ...
        for (i = 0; i != getPostCount(); i++) if (pins[i].side == SIDE_N || pins[i].side == SIDE_S) {
            hasVertical = true;
            break;
        }
        for (i = 0; i != getPostCount(); i++) {
            ...
            if (!hasVertical && sizeX > 2) availSpace = cspc * 2.5 + cspc * (sizeX - 3);
            while (true) {
                ...
                if (p.side == flippedXSide(SIDE_W)) tx = p.textloc.x - (cspc - 5);
                else if (p.side == flippedXSide(SIDE_E)) tx = p.textloc.x + (cspc - 5) - sw;
                else tx = p.textloc.x - sw / 2;
    ...
    void setPoints() {
        if (x2 - x > sizeX * cspc2 && this == sim.dragElm) setSize(2);
        int x0 = x + cspc2;
        int y0 = y;
        int xr = x0 - cspc;
        int yr = y0 - cspc;
        ...
        for (i = 0; i != getPostCount(); i++) {
            Pin p = pins[i];
            p.side = p.side0;
            if ((flags & FLAG_FLIP_XY) != 0) p.side = sideFlipXY[p.side];
            switch (p.side) {
                case SIDE_N:
    ...
    boolean getPinPos(int xp, int yp, int pin, int pos[]) {
        ...
        if ((pos[1] == SIDE_N || pos[1] == SIDE_S) && pos[0] >= sizeX) return false;
        if ((pos[1] == SIDE_W || pos[1] == SIDE_E) && pos[0] >= sizeY) return false;
        return true;
    }
    ...
    int getOverlappingPin(int p1, int p2, int pin) {
        for (int i = 0; i != getPostCount(); i++) {
            if (pin == i) continue;
            if (pins[i].overlaps(p1, p2)) return i;
        }
        return -1;
    }
    ...
    void stamp() {
        ...
        if (vsc != getVoltageSourceCount()) CirSim.console("voltage source count does not match number of outputs");
    }
    ...
    void doStep() {
        ...
        for (i = 0; i != getPostCount(); i++) {
            Pin p = pins[i];
            if (!p.output) p.value = volts[i] > getThreshold();
        }
        execute();
        for (i = 0; i != getPostCount(); i++) {
            Pin p = pins[i];
            if (p.output) sim.updateVoltageSource(0, nodes[i], p.voltSource, p.value ? highVoltage : 0);
        }
    }
    ...
    String dump() {
        ...
        String s = super.dump();
        if (needsBits()) s += " " + bits;
        if (hasCustomVoltage()) s += " " + highVoltage;
        int i;
        for (i = 0; i != getPostCount(); i++) {
            if (pins[i].state) s += " " + volts[i];
        }
        return s;
    }

    void writeOutput(int n, boolean value) {
        if (!pins[n].output) CirSim.console("pin " + n + " is not an output!");
        pins[n].value = value;
    }
    ...
    void setCurrent(int x, double c) {
        int i;
        for (i = 0; i != getPostCount(); i++) if (pins[i].output && pins[i].voltSource == x) pins[i].current = c;
    }
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (!isDigitalChip()) return getChipEditInfo(n - 3);

        if (n == 3) return new EditInfo("High Logic Voltage", highVoltage);
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (!isDigitalChip()) {
            if (n >= 3) setChipEditValue(n - 3, ei);
            return;
        }

        if (n == 3) highVoltage = ei.value;

        if (n >= 4) setChipEditValue(n - 4, ei);
    }
    ...
    static void readBits(StringTokenizer st, boolean[] output) {
        ...
        for (int i = 0; i < output.length; i++) {
            if (bitIndex >= Integer.SIZE) if (st.hasMoreTokens()) {
                integer = Integer.parseInt(st.nextToken()); //Load next integer
                bitIndex = 0;
            } else break; //Data is absent
    ...
    int flippedXSide(int s) {
        if ((flags & FLAG_FLIP_X) == 0) return s;
    ...
    class Pin {
        ...
        int toGrid(int p, int s) {
            ...
            if (s == SIDE_S) return p + sizeX * (sizeY - 1);
            if (s == SIDE_W) return p * sizeX;
            if (s == SIDE_E) return p * sizeX + sizeX - 1;
            return -1;
        }
