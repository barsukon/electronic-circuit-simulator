...
class Switch2Elm extends SwitchElm {
    ...
    void draw(Graphics g) {
        ...
        if (!(position == 2 && hasCenterOff())) drawDots(g, swpoles[position], swposts[position], curcount);
        drawPosts(g);
    }

    double getCurrentIntoNode(int n) {
        if (n == 0) return -current;
        if (n == position + 1) return current;
        return 0;
    }
    ...
    void calculateCurrent() {
        if (position == 2 && hasCenterOff()) current = 0;
    }

    void stamp() {
        if (
            position == 2 && hasCenterOff()
        ) return; // in center?
    ...
    boolean getConnection(int n1, int n2) {
        if (position == 2 && hasCenterOff()) return false;
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 1) return new EditInfo("Switch Group", link, 0, 100).setDimensionless();
        if (n == 2) return new EditInfo("# of Throws", throwCount, 2, 10).setDimensionless();
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 1) {
            ...
            if (ei.value >= 2) throwCount = (int) ei.value;
            if (throwCount > 2) momentary = false;
            allocNodes();
            setPoints();
