...
public class Scrollbar
    ...
    int calcValueFromPos(int x) {
        int v;
        v = min +
        ((max - min) * (x - HMARGIN - SCROLLHEIGHT - BARMARGIN)) / (CirSim.VERTICALPANELWIDTH - 2 * (HMARGIN + SCROLLHEIGHT + BARMARGIN));
        if (v < min) v = min;
        if (v > max) v = max;
        return v;
    }
        ...
        if (enabled) {
            if (x < HMARGIN + SCROLLHEIGHT) {
                ...
                if (x > CirSim.VERTICALPANELWIDTH - HMARGIN - SCROLLHEIGHT) {
                    ...
                    // don't get sent
                    if (mouse) Event.setCapture(can.getElement());
                }
            }
            draw();
    ...
    public void onMouseOut(MouseOutEvent e) {
        ...
        //		e.preventDefault();
        if (dragging) return;
        if (enabled && attachedElm != null && attachedElm.isMouseElm()) CircuitElm.sim.setMouseElm(null);
    }

    public void onMouseOver(MouseOverEvent e) {
        if (enabled && attachedElm != null) CircuitElm.sim.setMouseElm(attachedElm);
    }

    public void onMouseWheel(MouseWheelEvent e) {
        e.preventDefault();
        if (enabled) setValue(val + e.getDeltaY() / 3);
    }
