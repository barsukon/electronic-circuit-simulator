...
class SwitchElm extends CircuitElm {
    ...
    public SwitchElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) {
        ...
        if (str.compareTo("true") == 0) position = 1;
        else if (str.compareTo("false") == 0) position = 0;
        else position = new Integer(str).intValue();
        momentary = new Boolean(st.nextToken()).booleanValue();
        posCount = 2;
        label = null;
        if ((flags & FLAG_LABEL) != 0) label = CustomLogicModel.unescape(st.nextToken());
    }
    ...
    String dump() {
        ...
        if ((flags & FLAG_LABEL) != 0) s += " " + CustomLogicModel.escape(label);
        return s;
    }
    ...
    void draw(Graphics g) {
        ...
        if (position == 0) doDots(g);

        if (!needsHighlight()) g.setColor(whiteColor);
        ...
        if (label != null) {
            ...
            if (Math.abs(dy) > Math.abs(dx)) g.drawString(label, x + 10, (y < y2 ? lead1 : lead2).y - 5);
            else {
                g.save();
        ...
        if (useIECSymbol()) {
            ...
            if (momentary) g.drawLine(extraPoints[1], extraPoints[0]);
            else {
    ...
    void simpleToggle() {
        position++;
        if (position >= posCount) position = 0;
    }
        ...
        if (label != null) {
            ...
            for (i = 0; i != sim.elmList.size(); i++) {
                ...
                if (o instanceof SwitchElm && o != this) {
                    ...
                    if (label.equals(s2.label)) s2.simpleToggle();
                }
            }
        }
    }
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 1) return EditInfo.createCheckbox("IEC Symbol", useIECSymbol());
        if (n == 2) {
    ...
    public void setEditValue(int n, EditInfo ei) {
        if (n == 0) momentary = ei.checkbox.getState();
        if (n == 1) {
