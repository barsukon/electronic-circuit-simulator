...
class VoltageElm extends CircuitElm {
    ...
    void stamp() {
        if (waveform == WF_DC) sim.stampVoltageSource(nodes[0], nodes[1], voltSource, getVoltage());
        else sim.stampVoltageSource(nodes[0], nodes[1], voltSource);
    }

    void doStep() {
        if (waveform != WF_DC) sim.updateVoltageSource(nodes[0], nodes[1], voltSource, getVoltage());
    }

    void stepFinished() {
        if (waveform == WF_NOISE) noiseValue = (sim.random.nextDouble() * 2 - 1) * maxVoltage + bias;
    }

    double getVoltage() {
        if (waveform != WF_DC && sim.dcAnalysisFlag) return bias;
    ...
    void draw(Graphics g) {
        ...
        if (waveform == WF_DC) {
            ...
            drawWaveform(g, ps1);
            String inds;
            if (bias > 0 || (bias == 0 && waveform == WF_PULSE)) inds = "+";
            else inds = "*";
        ...
        updateDotCount();
        if (sim.dragElm != this) {
            if (waveform == WF_DC) drawDots(g, point1, point2, curcount);
            else {
    ...
    void drawWaveform(Graphics g, Point center) {
        ...
        int xc = center.x;
        int yc = center.y;
        if (waveform != WF_NOISE) drawThickCircle(g, xc, yc, circleSize);
        int wl = 8;
        ...
        switch (waveform) {
            ...
            case WF_AC: {
                ...
                for (i = -xl; i <= xl; i++) {
                    ...
                    if (i == -xl) g.context.moveTo(xc + i, yy);
                    else g.context.lineTo(xc + i, yy);
                }
                g.context.stroke();
        ...
        if (sim.showValuesCheckItem.getState() && waveform != WF_NOISE) {
            ...
            if (dx == 0 || dy == 0) drawValues(g, s, circleSize);
        }
    }
    ...
    void getInfo(String arr[]) {
        ...
        if (waveform != WF_DC && waveform != WF_VAR && waveform != WF_NOISE) {
            ...
            if (waveform == WF_AC && bias == 0) arr[i++] = "V(rms) = " + getVoltageText(maxVoltage / 1.41421356);
            if (bias != 0) arr[i++] = "Voff = " + getVoltageText(bias);
            else if (frequency > 500) arr[i++] = "wavelength = " + getUnitText(2.9979e8 / frequency, "m");
        }
        if (waveform == WF_DC && current != 0 && sim.showResistanceInVoltageSources) arr[i++] = "(R = " +
        getUnitText(maxVoltage / current, Locale.ohmString) +
        ")";
        arr[i++] = "P = " + getUnitText(getPower(), "W");
    }

    public EditInfo getEditInfo(int n) {
        if (n == 0) return new EditInfo(waveform == WF_DC ? "Voltage" : "Max Voltage", maxVoltage, -20, 20);
        if (n == 1) {
        ...
        if (n == 2) return new EditInfo("DC Offset (V)", bias, -20, 20);
        if (waveform == WF_DC || waveform == WF_NOISE) return null;
        if (n == 3) return new EditInfo("Frequency (Hz)", frequency, 4, 500);
        if (n == 4) return new EditInfo("Phase Offset (degrees)", (phaseShift * 180) / pi, -180, 180).setDimensionless();
        if (n == 5 && (waveform == WF_PULSE || waveform == WF_SQUARE)) return new EditInfo(
            "Duty Cycle",
            dutyCycle * 100,
            0,
            100
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 3) {
            ...
            if (frequency > maxfreq) {
                if (Window.confirm(Locale.LS("Adjust timestep to allow for higher frequencies?"))) sim.maxTimeStep = 1 / (32 * frequency);
                else frequency = maxfreq;
            }
        ...
        if (n == 1) {
            ...
            if (waveform == WF_DC && ow != WF_DC) {
                ei.newDialog = true;
                bias = 0;
            } else if (waveform != ow) ei.newDialog = true;

            // change duty cycle if we're changing to or from pulse
            if (waveform == WF_PULSE && ow != WF_PULSE) dutyCycle = defaultPulseDuty;
            else if (ow == WF_PULSE && waveform != WF_PULSE) dutyCycle = .5;

            setPoints();
        }
        if (n == 4) phaseShift = (ei.value * pi) / 180;
